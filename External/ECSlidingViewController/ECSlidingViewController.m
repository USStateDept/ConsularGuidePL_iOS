...
@property (nonatomic, strong) BilingualLabel *updatingLabel;
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (UIInterfaceOrientationIsLandscape(self.interfaceOrientation)) {
        self.anchorRightPeekAmount = self.view.bounds.size.width * 2.0f/3.0f;
    }
    else if (UIInterfaceOrientationIsPortrait(self.interfaceOrientation)) {
        self.anchorRightPeekAmount = self.view.bounds.size.width * 0.25f;
    }
}
...
//        self.anchorRightPeekAmount = self.view.bounds.size.height * 2.0f/3.0f;
//    }
...
- (void)removeTopViewSnapshot
{
    if (self.topViewSnapshot.superview) {
        [self.topViewSnapshot removeGestureRecognizer:self.topViewSnapshotPanGesture];
        self.topViewSnapshotPanGesture = nil;
        [self.topViewSnapshot removeGestureRecognizer:self.snapshotPanGesture];
...
- (void)showUpdateInProgressIndicator {
    ...
    self.updatingLabel.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
    self.updatingLabel.userInteractionEnabled = YES;
    
    self.updatingLabel.frame = self.view.bounds;
    [self.view addSubview:self.updatingLabel];
    
    self.snapshotPanGesture.enabled = NO;
    self.resetTapGesture.enabled = NO;
}

- (void)hideUpdateInProgressIndicator {
    self.snapshotPanGesture.enabled = YES;
    if (self.resetStrategy & ECTapping) {
        self.resetTapGesture.enabled = YES;
    }
    [self.updatingLabel removeFromSuperview];
    self.updatingLabel = nil;
...
- (void)setSnapshotPanGesture:(UIPanGestureRecognizer *)snapshotPanGesture {
    _snapshotPanGesture = snapshotPanGesture;
    if (self.updatingLabel) {
        _snapshotPanGesture.enabled = NO;
    }
    else {
        _snapshotPanGesture.enabled = YES;
    }
}

- (void)setResetTapGesture:(UITapGestureRecognizer *)resetTapGesture {
    _resetTapGesture = resetTapGesture;
    if (self.updatingLabel) {
        _resetTapGesture.enabled = NO;
    }
    else if (_resetStrategy & ECTapping) {
        _resetTapGesture.enabled = YES;
    }
}
